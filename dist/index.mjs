...
function useCloudStorage() {
  ...
  const getItems = useCallback((keys) => new Promise((resolve, reject) => {
    const wa = getWebApp();
    if (!(wa == null ? void 0 : wa.CloudStorage)) return reject(new Error("CloudStorage not available"));
    wa.CloudStorage.getItems(keys, (err, values) => {
      if (err) reject(err);
...
function useBiometric() {
  ...
  const init = useCallback(() => new Promise((resolve, reject) => {
    var _a;
    const bio = (_a = getWebApp()) == null ? void 0 : _a.BiometricManager;
    if (!bio) return reject(new Error("BiometricManager not available"));
    bio.init(() => {
  ...
  const requestAccess = useCallback((reason) => new Promise((resolve, reject) => {
    var _a;
    const bio = (_a = getWebApp()) == null ? void 0 : _a.BiometricManager;
    if (!bio) return reject(new Error("BiometricManager not available"));
    bio.requestAccess({ reason }, resolve);
  }), []);
  const authenticate = useCallback((reason) => new Promise((resolve, reject) => {
    var _a;
    const bio = (_a = getWebApp()) == null ? void 0 : _a.BiometricManager;
    if (!bio) return reject(new Error("BiometricManager not available"));
    bio.authenticate({ reason }, (authenticated, token) => resolve({ authenticated, token }));
...
function useHomeScreen() {
  ...
  const checkStatus = useCallback(() => new Promise((resolve, reject) => {
    const wa = getWebApp();
...
function TelegramProvider({
  ...
  return /* @__PURE__ */ jsx(TelegramContext.Provider, { value: ctx, children });
}
...
var FullscreenContext = createContext2(null);
function useFullscreen() {
  const ctx = useContext2(FullscreenContext);
...
function FullscreenProvider({
  ...
  const [isOrientationLocked, setIsOrientationLocked] = useState3(false);
  const [lastFullscreenError, setLastFullscreenError] = useState3(null);
  const [safeArea, setSafeArea] = useState3(DEFAULT_INSET);
  const [contentSafeArea, setContentSafeArea] = useState3(DEFAULT_INSET);
  const isInitializedRef = useRef2(false);
  const savePreference = useCallback3((value) => {
  ...
  const enterFullscreen = useCallback3(() => {
    const wa = getWebApp();
  ...
  const exitFullscreen = useCallback3(() => {
    const wa = getWebApp();
